      ...
      <div className="w-full py-4 sm:py-5">
        <div className="mb-6 overflow-hidden rounded-[36px] bg-[linear-gradient(135deg,#1f2937_0%,#0f766e_52%,#155e75_100%)] shadow-[0_24px_70px_rgba(0,0,0,0.18)]">
          <div className="grid gap-5 px-6 py-8 text-white sm:px-8 sm:py-9 lg:grid-cols-[minmax(0,1.8fr)_minmax(280px,1fr)] lg:items-end lg:px-10">
            <div>
              ...
              <h1 className="text-[34px] font-semibold leading-[1.08] tracking-[-0.04em] md:text-[48px]">{ieltsText('title')}</h1>
            ...
            <div className="grid grid-cols-3 gap-2">
              <div className="rounded-[24px] bg-sky-100/15 p-3.5 ring-1 ring-sky-100/30">
                <p className="text-[12px] font-semibold uppercase tracking-[0.12em] text-white/56">{ieltsText('years')}</p>
              ...
              <div className="rounded-[24px] bg-indigo-100/15 p-3.5 ring-1 ring-indigo-100/30">
                <p className="text-[12px] font-semibold uppercase tracking-[0.12em] text-white/56">{ieltsText('tests')}</p>
              ...
              <div className="rounded-[24px] bg-emerald-100/15 p-3.5 ring-1 ring-emerald-100/30">
                <p className="text-[12px] font-semibold uppercase tracking-[0.12em] text-white/56">{ieltsText('read')}</p>
        ...
        <div className="mb-5 grid gap-4 lg:grid-cols-[0.85fr_1.3fr_0.85fr]">
          <section className="rounded-[28px] border border-sky-100 bg-sky-50/55 p-5 shadow-[0_12px_40px_rgba(0,0,0,0.08)]">
            <p className="mb-3 text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">{ieltsText('stepOne')}</p>
            <h2 className="mb-3 text-lg font-semibold text-slate-900">{ieltsText('chooseYear')}</h2>
          ...
          <section className="rounded-[28px] border border-indigo-100 bg-indigo-50/55 p-5 shadow-[0_12px_40px_rgba(0,0,0,0.08)]">
            <p className="mb-3 text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">{ieltsText('stepTwo')}</p>
            <h2 className="mb-3 text-lg font-semibold text-slate-900">{ieltsText('chooseMonth')}</h2>
            <div className="flex flex-wrap gap-2">
          ...
          <section className="rounded-[28px] border border-emerald-100 bg-emerald-50/55 p-5 shadow-[0_12px_40px_rgba(0,0,0,0.08)]">
            <p className="mb-3 text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">{ieltsText('stepThree')}</p>
            <h2 className="mb-3 text-lg font-semibold text-slate-900">{ieltsText('chooseTest')}</h2>
            <div className="flex flex-wrap gap-2">
              {availableTests.map((test) => (
                ...
                >
                  {formatMessage(ieltsText('testLabel'), { test })}
                </button>
              ))}
            </div>
          </section>
                  ...
                  >
                    <div className="mb-2 flex items-center justify-between gap-3">
                      <span className="rounded-full bg-sky-100 px-3 py-1 text-xs font-semibold uppercase tracking-[0.14em] text-sky-700">
                        {formatMessage(ieltsText('passage'), { passage: article.passage })}
                      </span>
                      {isRead && (
                    ...
                    <div className="mt-3 flex flex-wrap gap-3 text-sm text-slate-500">
                      <span>{formatMessage(ieltsText('words'), { count: article.wordCount.toLocaleString() })}</span>
                      <span>{formatMessage(ieltsText('readingTime'), { count: article.readingTime })}</span>
                    </div>

                    <div className="mt-4 inline-flex items-center text-sm font-semibold text-sky-700">
                      {ieltsText('openPassage')}
                    </div>
                  </Link>
                );
              })}
